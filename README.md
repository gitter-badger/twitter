...
1) Twitter is a directed graph of followed accounts and their followers. A followed account will be called a 'source', and a follower of a source will be called a 'sink'. For nodes `x` and `y`, `(x -> y)` will denote the relationship in which `x` is the source and `y` a sink. 
...
In words: 
  ...
  + Additionally, we will profile the performance of every Twitter implementation, seeing in which conditions our service will fail
